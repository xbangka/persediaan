...
$x = $_POST['kodebrg'];
$q = mysqli_query($jazz,"SELECT * FROM barang WHERE kodebrg = '$x'");
$rs = mysqli_fetch_array($q);
	
$pdf->Ln(10);
$pdf->SetX(5);
$pdf->SetFont('Times','B',12);
$pdf->Cell(200,5,'LAPORAN MUTASI PER BARANG',0,0,'L');

$pdf->Ln(5);
$pdf->SetFont('Times','',12);
$pdf->SetX(5);
$pdf->Cell(25, 5,'Kode Barang', 0, 0, "L");
$pdf->Cell(175, 5,': '.$rs['kodebrg'], 0, 0, "L");
$pdf->Ln(5);
$pdf->SetX(5);
$pdf->Cell(25, 5,'Nama Barang', 0, 0, "L");
$pdf->Cell(125, 5,': '.$rs['brg'] , 0, 0, "L");
$pdf->Cell(30, 5,'Satuan Barang :' , 0, 0, "R");
$pdf->Cell(20, 5,$rs['satuan'] , 0, 0, "R");
$pdf->Ln(5);
$pdf->SetX(5);
$pdf->Cell(25, 5,'Periode', 0, 0, "L");
$pdf->Cell(125, 5,': '.$tgl1.'  s.d  '.$tgl2 , 0, 0, "L");
$pdf->Cell(30, 5,'Harga :' , 0, 0, "R");
$pdf->Cell(20, 5,number_format($rs['harga'], 0, '', '.') , 0, 0, "R");
#    ============================ Title ============================
...
$pdf->Cell(25,8,'No. Mutasi',1,0,'C',1);
$pdf->Cell(20,8,'Tanggal',1,0,'C',1);
$pdf->Cell(108,8,'Keterangan',1,0,'C',1);
$pdf->Cell(12,8,'Masuk',1,0,'C',1);
$pdf->Cell(12,8,'Keluar',1,0,'C',1);
$pdf->Cell(12,8,'Sisa',1,0,'C',1);
#    =========================== Header Table ===========================
...
$sql = mysqli_query($jazz,"SELECT count(jml) AS stok_masuk
						  FROM mutasi 
						  WHERE tgl < '$tgl1' AND kodebrg = '$x' AND mutasi = '1'");
$data = mysqli_fetch_array($sql);

$stok_masuk = $data['stok_masuk'] ;


$sql = mysqli_query($jazz,"SELECT count(jml) AS stok_keluar
						  FROM mutasi 
						  WHERE tgl < '$tgl1' AND kodebrg = '$x' AND mutasi = '2'");
$data = mysqli_fetch_array($sql);

$stok_keluar = $data['stok_keluar'] ;

$stok_awal = number_format( $stok_masuk - $stok_keluar , 0, '', '.') ;
...
$pdf->Cell(132,5,'Stok Awal',1,0,'L');
$pdf->SetFont('Times','',10);
$pdf->Cell(12,5,$stok_awal,1,0,'R');
...
$sql = mysqli_query($jazz,"SELECT 
						  ...
						  WHERE 
						    m.tgl >= '$tgl1' AND m.tgl <= '$tgl2' AND b.kodebrg = '$x'
						  ORDER BY m.nomutasi ASC");
while($data = mysqli_fetch_array($sql)){
	...
	if($n % 2 == 0){
		...
		$pdf->Cell(25,5,$data[1],1,0,'C');
		$pdf->Cell(20,5,$data[2],1,0,'C');
		$pdf->Cell(108,5,$data[5],1,0,'L');
		$pdf->Cell(12,5,$strMasuk,1,0,'R');
		$pdf->Cell(12,5,$strKeluar,1,0,'R');
		$pdf->Cell(12,5,$sisa,1,0,'R');
	}else{
		$pdf->Ln(5);
		$pdf->SetX(6);
		$pdf->SetFont('Times','',10);
		...
		$pdf->Cell(25,5,$data[1],1,0,'C',1);
		$pdf->Cell(20,5,$data[2],1,0,'C',1);
		$pdf->Cell(108,5,$data[5],1,0,'L',1);
		$pdf->Cell(12,5,$strMasuk,1,0,'R',1);
		$pdf->Cell(12,5,$strKeluar,1,0,'R',1);
		$pdf->Cell(12,5,$sisa,1,0,'R',1);
	}
