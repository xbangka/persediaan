...
include("libraries/PDF/FPDF.php");
# ---------------------------------
...
$pdf->Cell(200,5,'REKAPITULASI MUTASI BARANG',0,0,'L');
...
$pdf->Cell(22,8,'Kode Barang',1,0,'C',1);
$pdf->Cell(65,8,'Nama Barang',1,0,'C',1);
$pdf->Cell(13,8,'Satuan',1,0,'C',1);
$pdf->Cell(17,8,'Harga',1,0,'C',1);
$pdf->Cell(15,8,'Tersedia',1,0,'C',1);
$pdf->Cell(17,8,'Stok Awal',1,0,'C',1);
$pdf->Cell(12,8,'Masuk',1,0,'C',1);
$pdf->Cell(12,8,'Keluar',1,0,'C',1);
$pdf->Cell(18,8,'Stok Akhir',1,0,'C',1);
$y = 0 ;
#    =========================== Header Table ===========================
...
while($data = mysqli_fetch_array($sql)){
	...
	$Masuk 		= $Masuk + $data[6] ;
	$Keluar 	= $Keluar + $data[7] ;
	$stok_akhir	= $stok_akhir + $data[8] ;
	
	
	$data[4] = number_format($data[4], 0, '', '.');
	$data[6] = number_format($data[6], 0, '', '.');
	$data[7] = number_format($data[7], 0, '', '.');
	$data[8] = number_format($data[8], 0, '', '.');
	...
	$sql2 = mysqli_query($jazz,"SELECT 
						  ...
						  WHERE 
						    m.tgl < '$tgl1' AND b.kodebrg = '$data[1]'");
	$data2 = mysqli_fetch_array($sql2);
	
	$stok_awal 	= $stok_awal + $data2['stok_awal'] ;
	...
	if($n % 2 == 1){
		...
		$pdf->Cell(8,5,$n,1,0,'R');
		$pdf->Cell(22,5,$data[1],1,0,'C');
		$pdf->Cell(65,5,$data[2],1,0,'L');
		$pdf->Cell(13,5,$data[3],1,0,'C');
		$pdf->Cell(17,5,$data[4],1,0,'R');
		$pdf->Cell(15,5,$data[5],1,0,'C');
		$pdf->Cell(17,5,number_format($data2['stok_awal'], 0, '', '.'),1,0,'R');
		$pdf->Cell(12,5,$data[6],1,0,'R');
		$pdf->Cell(12,5,$data[7],1,0,'R');
		$pdf->Cell(18,5,$data[8],1,0,'R');
	}else{
		$pdf->Ln(5);
		$pdf->SetX(6);
		$pdf->SetFont('Times','',10);
		$pdf->setFillColor(230,230,230); 
		$pdf->Cell(8,5,$n,1,0,'R',1);
		$pdf->Cell(22,5,$data[1],1,0,'C',1);
		$pdf->Cell(65,5,$data[2],1,0,'L',1);
		$pdf->Cell(13,5,$data[3],1,0,'C',1);
		$pdf->Cell(17,5,$data[4],1,0,'R',1);
		$pdf->Cell(15,5,$data[5],1,0,'C',1);
		$pdf->Cell(17,5,number_format($data2['stok_awal'], 0, '', '.'),1,0,'R',1);
		$pdf->Cell(12,5,$data[6],1,0,'R',1);
		$pdf->Cell(12,5,$data[7],1,0,'R',1);
		$pdf->Cell(18,5,$data[8],1,0,'R',1);
	}
...
$pdf->Cell(140,7,'Jumlah',1,0,'C',1);

$pdf->Cell(17,7,number_format($stok_awal, 0, '', '.'),1,0,'R',1);
$pdf->Cell(12,7,number_format($Masuk, 0, '', '.'),1,0,'R',1);
$pdf->Cell(12,7,number_format($Keluar, 0, '', '.'),1,0,'R',1);
$pdf->Cell(18,7,number_format($stok_akhir, 0, '', '.'),1,0,'R',1);
