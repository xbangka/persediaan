...
$sql2 = mysqli_query($jazz,"SELECT 
						...
						pemohon.nama
					FROM permintaan 
					INNER JOIN pemohon ON pemohon.user_id = permintaan.user_approve 
...
$sql = mysqli_query($jazz,"SELECT rinci_permintaan.* , barang.satuan FROM rinci_permintaan INNER JOIN barang ON rinci_permintaan.kodebrg = barang.kodebrg WHERE rinci_permintaan.idx='$rs[x]' ORDER BY rinci_permintaan.x ASC");
...
$sql2 = mysqli_query($jazz,"SELECT keterangan FROM pemohon WHERE user_id = '$rs[user_approve]'");
...
$filebarcode = 'barcode/'.strtolower($kodepm).'.png';
