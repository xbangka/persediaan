...
if (defined('CLASS_FPDF')) {
	return 0;
} else {
	define('CLASS_FPDF', TRUE);
}
...
var $InFooter;           //flag set when processing 
...
/*function Image($file,$x,$y,$w,$h,$type='',$link='')
{ print $link ;
	//Put an image on the page
	if(!isset($this->images[$file]))
	{
...
function konvnominal1($nom,$jmlchr)
{
	switch ($jmlchr)
	{			
		case 1: //Null
			$first=substr($nom,0,1);
			$nomin=$first;
			return konvnominal1($nom,$jmlchr);
			//print_r("$nomin");
			break;
		
		case 2:
			$first=substr($nom,0,2);
			$nomin=$first;
			return konvnominal1($nom,$jmlchr);
			//print_r("$nomin");
			break;
		...
		case 10: //Milyaran
			...
			$mid=substr($nom,1,3);
			$end=substr($nom,4,3);
			$finish=substr($nom,7,3);
			$nomin=$first.",".$mid.",".$end.",".$finish;
			return konvnominal1($nom,$jmlchr);
			//print_r("$nomin");
			break;		
	}	
